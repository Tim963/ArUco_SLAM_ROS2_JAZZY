...
class ArucoMapper(Node):
    def __init__(self):
        ...
        self.marker_graph = nx.Graph()  # Für vernetzte Korrekturen
        ...
        self.get_logger().info("Aruco Mapper mit vernetzter Driftkorrektur gestartet")
    ...
    def record_relative_positions(self, current_transforms):
        ...
        for i in range(len(marker_ids)):
            for j in range(i+1, len(marker_ids)):
                ...
                self.marker_graph.add_edge(id1, id2, weight=np.linalg.norm(T_rel[:3,3]))
    ...
    def global_drift_correction(self, visible_markers):
        ...
        try:
            ...
            if pair_errors[worst_pair] > self.drift_threshold:
                ...
                # 3. Berechne Korrektur für das gesamte Netzwerk
                corrections = {}
                ref_id = worst_pair[0]
                
                for marker_id in self.marker_map:
                    if marker_id == ref_id:
                        continue
                    
                    try:
                        # Finde kürzesten Pfad zum Referenzmarker
                        path = nx.shortest_path(self.marker_graph, source=ref_id, target=marker_id)
                        
                        # Berechne erwartete Transformation entlang des Pfades
                        T_expected = np.eye(4)
                        for i in range(len(path)-1):
                            a, b = path[i], path[i+1]
                            if (a,b) in self.observed_pairs:
                                T_rel = np.mean(self.observed_pairs[(a,b)][-5:], axis=0)
                            else:
                                T_rel = np.mean(self.observed_pairs[(b,a)][-5:], axis=0)
                                T_rel = np.linalg.inv(T_rel)
                            T_expected = T_expected @ T_rel
                        
                        # Berechne Korrektur
                        T_current = self.marker_map[marker_id]
                        correction = T_expected @ np.linalg.inv(T_current)
                        corrections[marker_id] = correction
                    except:
                        continue
                
                # 4. Wende Korrekturen an (gewichtete Mittelung)
                for marker_id, correction in corrections.items():
                    # Nur Translation korrigieren (Rotation stabiler)
